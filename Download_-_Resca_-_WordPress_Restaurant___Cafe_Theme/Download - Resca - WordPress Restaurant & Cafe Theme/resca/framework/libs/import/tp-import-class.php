    ...
    function import_woosetting($file) {
        ...
        foreach ($keys as $key) {
			if($option_data = unserialize($woo_datas[$key])){
				...
				$format = __( 'Update Option "%d" error' );
...
//
//        update_option('theme_mods_$name', $setting_data);
//
//        return true;
//    }
    ...
    function set_options($option_file) {
        ...
        $setting_data = json_decode(file_get_contents($option_file), true);
    ...
    function set_menus() {
        ...
        @$menu_nav_settings = file_get_contents(MENU_CONFIG);
        $menu_reading_settings = file_get_contents(MENU_READING_CONFIG);
    ...
    function import_widgets($import_array) {
        ...
        if (is_file($widgets_logic_file)) {
            $import = explode("\n", file_get_contents($widgets_logic_file, false));
