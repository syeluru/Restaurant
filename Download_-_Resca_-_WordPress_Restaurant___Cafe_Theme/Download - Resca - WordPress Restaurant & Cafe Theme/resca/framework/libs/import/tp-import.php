...
if( $type=='download' ) {
	...
	if( isset( $demo_datas[$demodata] ) ) {
			...
			if( is_dir( $data_dir ) )
			{
				...
				if( !wp_mkdir_p( $demodata_dir )) {
					$response = array(
						...
						'next'=>'extract',
						'return'=>false,
						'message'=>sprintf( __('Cannot create directory "%"'), $demodata_dir ),
					);
		...
		if( !file_put_contents( $demodata_dir.DIRECTORY_SEPARATOR.$filename, $file_content ) ){
			$response = array(
							...
							'next'=>'extract',
							'return'=>false,
							'message'=>sprintf( __('Cannot store file "%s"'), $demodata_dir.DIRECTORY_SEPARATOR.$filename ),
						);
			echo json_encode($response);
			exit();
		}
		...
		$response = array(
						...
						'next'=>'extract',
						'return'=>true,
						'message'=>sprintf( __('Download demo data package success "%s"'), $demodata_dir.DIRECTORY_SEPARATOR.$filename ),
						'log'=>$log,
					);
		echo json_encode($response);
		exit();
	...
	if ( $unzipfile === true  ) {
		...
		$response = array(
						...
						'next'=>$next_step,
						'return'=>true,
						'message'=>sprintf( __('Extract demo data package to "%s" success'), $unzip_dir ),
						'log'=>$log,
					);
		echo json_encode($response);
		exit();
			...
			switch ( trim( $type ) ) {
				case 'woo_setting':
					...
					$response = array(
						...
						'next'=>'core',
						'return'=>true,
						'message'=>__('Import woo setting success','thim'),
						'logs'=>$log,
					);
					echo json_encode($response);
//					echo 'core';
...
//					echo 'widgets';
				...
				case 'menus':
					$next_step = is_file( REV_IMPORT )?'slider':'done';
