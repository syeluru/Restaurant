...
if ( class_exists( 'WP_Importer' ) ) {
	...
	class WP_Import extends WP_Importer {
		...
		function updateTaxCount() {
			...
			_update_generic_term_count( $arg_tax, '' );
		}
		...
		function import( $file ) {
			...
			@$this->processed_menu_items = json_decode( file_get_contents( MENU_MAPPING ), true );
			@$this->processed_terms = json_decode( file_get_contents( PROCESS_TERM ), true );
			@$this->processed_posts = json_decode( file_get_contents( PROCESS_POSTS ), true );
			@$this->menu_item_orphans = json_decode( file_get_contents( MENU_ITEM_ORPHANS ), true );
			@$this->missing_menu_items = json_decode( file_get_contents( MENU_MISSING ), true );
			@$this->url_remap = json_decode( file_get_contents( URL_REMAP ), true );
			@$this->post_orphans = json_decode( file_get_contents( POST_ORPHANS ), true );
//			@$this->featured_images = json_decode( file_get_contents( FEATURE_IMAGES ), true );

//			@$this->processed_menu_items = json_decode( $_SESSION['processed_menu_items'], true );
//			@$this->processed_terms = json_decode( $_SESSION['processed_terms'], true );
//			@$this->processed_posts = json_decode( $_SESSION['processed_posts'], true );
//			@$this->menu_item_orphans = json_decode( $_SESSION['menu_item_orphans'], true );
//			@$this->missing_menu_items = json_decode( $_SESSION['missing_menu_items'], true );
//			@$this->url_remap = json_decode( $_SESSION['url_remap'], true );
//			@$this->post_orphans = json_decode( $_SESSION['post_orphans'], true );
//			@$this->featured_images = json_decode( $_SESSION['featured_images'], true );
			...
			if ( !$this->process_posts() ) {
				if ( $this->processed_menu_items ) {
					file_put_contents( MENU_MAPPING, json_encode( $this->processed_menu_items ) );
//					$_SESSION['processed_menu_items'] = json_encode( $this->processed_menu_items );
				}
				if ( $this->processed_terms ) {
					file_put_contents( PROCESS_TERM, json_encode( $this->processed_terms ) );
//					$_SESSION['processed_terms'] = json_encode( $this->processed_terms );
				}
				if ( $this->processed_posts ) {
					file_put_contents( PROCESS_POSTS, json_encode( $this->processed_posts ) );
//					$_SESSION['processed_posts'] = json_encode( $this->processed_posts );
				}
				if ( $this->menu_item_orphans ) {
					file_put_contents( MENU_ITEM_ORPHANS, json_encode( $this->menu_item_orphans ) );
//					$_SESSION['menu_item_orphans'] = json_encode( $this->menu_item_orphans );
				}
				if ( $this->missing_menu_items ) {
					file_put_contents( MENU_MISSING, json_encode( $this->missing_menu_items ) );
//					$_SESSION['missing_menu_items'] = json_encode( $this->missing_menu_items );
				}
				if ( $this->url_remap ) {
					file_put_contents( URL_REMAP, json_encode( $this->url_remap ) );
//					$_SESSION['url_remap'] = json_encode( $this->url_remap );
				}
				if ( $this->post_orphans ) {
					file_put_contents( POST_ORPHANS, json_encode( $this->post_orphans ) );
//					$_SESSION['post_orphans'] = json_encode( $this->post_orphans );
				}
//				if ( $this->featured_images ) {
//					file_put_contents( FEATURE_IMAGES, json_encode( $this->featured_images ) );
////					$_SESSION['featured_images'] = json_encode( $this->featured_images );
//				}
...
//			unset( $_SESSION['processed_menu_items'] );
//			unset( $_SESSION['processed_terms'] );
//			unset( $_SESSION['processed_posts'] );
//			unset( $_SESSION['menu_item_orphans'] );
//			unset( $_SESSION['missing_menu_items'] );
//			unset( $_SESSION['url_remap'] );
//			unset( $_SESSION['post_orphans'] );
//			unset( $_SESSION['featured_images'] );
//			unset( $_SESSION['k'] );
		...
		function import_start( $file ) {
			if ( !is_file( $file ) ) {
				////echo '<p><strong>' . __( 'Sorry, there has been an error.', THEME_NAME ) . '</strong><br />';
		...
		function import_end() {
			...
			//echo '<p>' . __( 'All done.', THEME_NAME ) . ' <a href="' . admin_url() . '">' . __( 'Have fun!', THEME_NAME ) . '</a>' . '</p>';
			//echo '<p>' . __( 'Remember to update the passwords and roles of imported users.', THEME_NAME ) . '</p>';
		...
		function import_options() {
			...
			<form action="<?php echo admin_url( 'admin.php?import=wordpress&amp;step=2' ); ?>" method="post">
				...
				<?php if ( !empty( $this->authors ) ) : ?>
					<h3><?php esc_attr_e( 'Assign Authors', THEME_NAME ); ?></h3>
					<p><?php esc_attr_e( 'To make it easier for you to edit and save the imported content, you may want to reassign the author of the imported item to an existing user of this site. For example, you may want to import all the entries as <code>admin</code>s entries.', THEME_NAME ); ?></p>
					<?php if ( $this->allow_create_users() ) : ?>
						<p><?php //printf( __( 'If a new user is created by WordPress, a new password will be randomly generated and the new user&#8217;s role will be set as %s. Manually changing the new user&#8217;s details will be necessary.', THEME_NAME ), esc_html( get_option( 'default_role' ) ) );    ?></p>
					<?php endif; ?>
					<ol id="authors">
				...
				<?php if ( $this->allow_fetch_attachments() ) : ?>
					<h3><?php esc_attr_e( 'Import Attachments', THEME_NAME ); ?></h3>
					<p>
						<input type="checkbox" value="1" name="fetch_attachments" id="import-attachments" />
						<label for="import-attachments"><?php esc_attr_e( 'Download and import file attachments', THEME_NAME ); ?></label>
					</p>
				<?php endif; ?>

				<p class="submit">
					<input type="submit" class="button" value="<?php esc_attr_e( 'Submit', THEME_NAME ); ?>" /></p>
			</form>
		<?php
		}
		...
		function author_select( $n, $author ) {
			...
			if ( $create_users ) {
				if ( $this->version != '1.0' ) {
					esc_attr_e( 'or create new user with login name:', THEME_NAME );
					$value = '';
				} else {
					esc_attr_e( 'as a new user:', THEME_NAME );
			...
			if ( !$create_users && $this->version == '1.0' ) {
				esc_attr_e( 'assign posts to an existing user:', THEME_NAME );
			} else {
				esc_attr_e( 'or assign posts to an existing user:', THEME_NAME );
			}
			wp_dropdown_users( array( 'name' => "user_map[$n]", 'multi' => true, 'show_option_all' => __( '- Select -', THEME_NAME ) ) );
		...
		function process_posts() {
			...
			//$this->posts = $this->posts_segment();
			@$start = intval( file_get_contents( POST_COUNT ) );
//			@$start = intval( $_SESSION['k'] );
			$end = 0;
			if ( $start < 1 ) {
				$start = 0;
				$end   = 50;
...
//				$_SESSION['k'] = $k;
				...
				file_put_contents( POST_COUNT, $k );
...
//											'!s:(\d+):"(.*?)";!se', "'s:'.strlen('$2').':\"$2\";'", $meta['value']
//										)
//									);
//								}
...
//								$this->featured_images[$post_id] = (int) $value;
		...
		function process_attachment( $post, $url ) {
			if ( !$this->fetch_attachments ) {
				return new WP_Error( 'attachment_processing_error', __( 'Fetching attachments is not enabled', THEME_NAME ) );
			}
...
//			}
		...
		function header() {
			//echo '<div class="wrap">';
			//screen_icon();
			echo '<h2>' . __( 'Import WordPress', THEME_NAME ) . '</h2>';
			...
			if ( isset( $updates[$basename] ) ) {
				...
				//echo '<div class="error"><p><strong>';
				//printf( __( 'A new version of this importer is available. Please update to version %s to ensure compatibility with newer export files.', THEME_NAME ), $update->update->new_version );
				//echo '</strong></p></div>';
			}
		}
		...
		function greet() {
			...
			//echo '<p>' . __( 'Howdy! Upload your WordPress eXtended RSS (WXR) file and we&#8217;ll import the posts, pages, comments, custom fields, categories, and tags into this site.', THEME_NAME ) . '</p>';
			//echo '<p>' . __( 'Choose a WXR (.xml) file to upload, then click Upload file and import.', THEME_NAME ) . '</p>';
...
//							print_r($image_path_on_upload);die;
							...
							if ( file_exists( $image_path ) ) {
								//echo 'file in local: ' . $image_path . '<br/>';
								...
								//echo 'demo in local: ' . $image_path . '<br/>';
...
if ( !function_exists( 'wordpress_importer_init' ) ) {
	...
	function wordpress_importer_init() {
		...
		register_importer( 'wordpress', 'WordPress', __( 'Import <strong>posts, pages, comments, custom fields, categories, and tags</strong> from a WordPress export file.', THEME_NAME ), array( $GLOBALS['wp_import'], 'dispatch' ) );
	}
