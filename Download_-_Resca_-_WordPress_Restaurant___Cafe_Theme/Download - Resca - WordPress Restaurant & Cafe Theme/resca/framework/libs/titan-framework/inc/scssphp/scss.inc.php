        ...
        foreach ($selector as $i => $part) {
            ...
            if ($this->matchExtendsSingle($part, $origin)) {
                ...
                foreach ($origin as $new) {
                    ...
                    $result = array_merge(
                        $before, $k > 0 ? array_slice($new, $k) : $new, $after);
                    ...
                    if (!empty($before) && count($new) > 1) {
                        $result2 = array_merge(
                            array_slice($new, 0, -1), $k > 0 ? array_slice($before, $k) : $before, array_slice($new, -1), $after);
        ...
        switch ($child[0]) {
            ...
            case "assign":
                ...
                $out->lines[] = $this->formatter->property(
                    $this->compileValue($name), $compiledValue);
                break;
            case "comment":
                $out->lines[] = $child[1];
                break;
...
class scss_parser {
    ...
    static protected function makeOperatorStr($operators) {
        return '(' . implode('|', array_map(array('scss_parser', 'preg_quote'), $operators)) . ')';
    }
...
class scss_formatter {
    ...
    protected function block($block) {
        ...
        if (!empty($block->lines)) {
            ...
            echo ent2ncr($inner . implode($glue, $block->lines));
            if (!empty($block->children)) {
                echo ent2ncr($this->break);
            }
        }
        ...
        if (!empty($block->selectors)) {
            ...
            if (empty($block->children))
                echo ent2ncr($this->break);
            echo ent2ncr($pre . $this->close . $this->break);
        }
    }
