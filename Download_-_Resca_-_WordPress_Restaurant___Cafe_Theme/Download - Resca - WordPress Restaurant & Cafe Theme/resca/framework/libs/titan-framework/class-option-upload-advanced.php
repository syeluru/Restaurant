		...
		if ( ! is_array( $value ) ) {
			...
			foreach ($value as $k => $v) { 
				...
				if ( ! empty( $v ) ) {
					...
					$previewImage = "<i class='dashicons dashicons-no-alt remove'></i><img src='" . esc_url( $v[0] ) . "' style='display: none'/>";
				}
	...
	public static function createUploaderScript() {
		...
		jQuery(document).ready(function($){
			...
			$('body').on('click', '.tf-upload-advanced .thumbnail, .tf-upload-advanced img', function(event) {
				...
				var frame = wp.media({
					title: '<?php esc_attr_e( 'Select Image', TF_I18NDOMAIN ) ?>',
					multiple: true,
					library: { type: 'image' },
					button : { text : '<?php esc_attr_e( 'Use image', TF_I18NDOMAIN ) ?>' }
				});
