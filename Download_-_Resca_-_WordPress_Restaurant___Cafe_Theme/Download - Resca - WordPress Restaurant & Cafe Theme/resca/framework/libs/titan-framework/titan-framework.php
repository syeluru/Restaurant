...
/* Custom font */
require_once( TF_PATH . 'class-option-font-color.php' );
require_once( TF_PATH . 'class-option-color-opacity.php' );
...
/* Custom Upload */
require_once( TF_PATH . 'class-option-upload-advanced.php' );
...
require_once( TF_PATH . 'class-option-customize-import.php' );
require_once( TF_PATH . 'class-option-customize-export.php' );
require_once( TF_PATH . 'class-import-export-customizer.php' );
require_once( TF_PATH . '/export-settings.php' );
require_once( TF_PATH . 'thim-functions.php' );
...
class TitanFrameworkPlugin {
    ...
    public function pluginLinks($plugin_meta, $plugin_file) {
        if ($plugin_file == plugin_basename(__FILE__)) {
            $plugin_meta[] = sprintf("<a href='%s' target='_blank'>%s</a>", "http://www.titanframework.net/docs", __("Documentation", TF_I18NDOMAIN)
            );
            $plugin_meta[] = sprintf("<a href='%s' target='_blank'>%s</a>", "https://github.com/gambitph/Titan-Framework", __("GitHub Repo", TF_I18NDOMAIN)
            );
            $plugin_meta[] = sprintf("<a href='%s' target='_blank'>%s</a>", "https://github.com/gambitph/Titan-Framework/issues", __("Issue Tracker", TF_I18NDOMAIN)
            );
        }
        return $plugin_meta;
    }
