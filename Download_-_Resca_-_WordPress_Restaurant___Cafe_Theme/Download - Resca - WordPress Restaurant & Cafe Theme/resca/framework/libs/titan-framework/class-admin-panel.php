...
class TitanFrameworkAdminPanel {
	...
	public function saveOptions() {
		...
		$url = wp_get_referer();
		$activeTab = $this->getActiveTab();
		$url = esc_url(add_query_arg( 'page', urlencode( $this->settings['id'] ), $url ));
		if ( ! empty( $activeTab ) ) {
			$url = esc_url(add_query_arg( 'tab', urlencode( $activeTab->settings['id'] ), $url ));
		}
		if ( ! empty( $message ) ) {
			$url = esc_url(add_query_arg( 'message', $message, $url ));
		}
	...
	public function createAdminPage() {
		...
		if ( ! empty( $this->settings['desc'] ) ) {
			?><p class='description'><?php echo ent2ncr($this->settings['desc'])  ?></p><?php
		}
		?>
		...
		if ( ! empty( $activeTab ) ) {
			...
			if ( ! empty( $activeTab->settings['desc'] ) ) {
				?><p class='description'><?php echo ent2ncr($activeTab->settings['desc']) ?></p><?php
			}
