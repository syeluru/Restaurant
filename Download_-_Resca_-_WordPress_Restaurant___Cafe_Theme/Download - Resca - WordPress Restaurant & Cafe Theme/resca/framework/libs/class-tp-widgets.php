...
abstract class Thim_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		@ include TP_THEME_DIR . 'inc/widgets/' . $this->id_base . '/tpl/' . $this->get_template_name( $instance ) . '.php';
		echo '</div>';
	...
	public function form( $instance ) {
		...
		<?php if ( !empty( $this->widget_options['help'] ) ) : ?>
			<a href="<?php echo esc_url( $this->widget_options['help'] ) ?>" class="thim-widget-help-link thim-panels-help-link" target="_blank"><?php esc_attr_e( 'Help', 'thim' ) ?></a>
		<?php endif; ?>
		...
		<script type="text/javascript">
			(function ($) {
				...
				if (!$('#thim-widget-admin-css').length && $.isReady) {
					alert('<?php esc_attr_e('Please refresh this page to start using this widget.', 'thim') ?>')
				}
			})(jQuery);
		</script>
	<?php
	...
	function enqueue_scripts() {
		...
		if ( !wp_script_is( 'thim-widget-admin-posts-selector' ) && $this->using_posts_selector() ) {
			...
			wp_localize_script( 'thim-widget-admin-posts-selector', 'obPostsSelectorTpl', array(
				...
				'foundPosts'  => '<div class="ob-post-count-message">' . sprintf( __( 'This query returns <a href="#" class="preview-query-posts">%s posts</a>.', 'thim' ), '<%= foundPosts %>' ) . '</div>',
			...
			wp_localize_script( 'thim-widget-admin-posts-selector', 'obPostsSelectorVars', array(
				'modalTitle' => __( 'Select Posts', 'thim' ),
			) );
		}
	...
	public function get_instance_css( $instance ) {
		...
		$less = file_get_contents( TP_THEME_DIR . 'inc/widgets/' . $this->id_base . '/style/' . $style_name . '.less' );
		...
		//get mixins
		$mixins = file_get_contents( TP_THEME_DIR . 'less/mixins.less' );
	...
	function render_field( $name, $field, $value, $repeater = array(), $is_template = false ) {
		...
		if ( $field['type'] != 'repeater' && $field['type'] != 'checkbox' && $field['type'] != 'separator' && !empty( $field['label'] ) ) {
			...
			<label for="<?php echo esc_attr( $field_id ) ?>" class="thim-widget-field-label <?php if ( empty( $field['hide'] ) ) {
				...
				if ( !empty( $field['optional'] ) ) {
					echo ' <span class="field-optional">(' . __( 'Optional', 'thim' ) . ')</span>';
				}
				?>
			</label>
		<?php
		...
		switch ( $field['type'] ) {
			...
			case 'textarea' :
				?>
				<textarea type="text" name="<?php echo esc_attr( $this->so_get_field_name( $name, $repeater ) ) ?>" id="<?php echo esc_attr( $this->so_get_field_id( $name, $repeater ) ) ?>" class="widefat thim-widget-input" rows="<?php echo !empty( $field['rows'] ) ? intval( $field['rows'] ) : 4 ?>"><?php echo esc_textarea( $value ) ?></textarea><?php
				break;
			...
			case 'media':
				if ( version_compare( get_bloginfo( 'version' ), '3.5', '<' ) ) {
					printf( __( 'You need to <a href="%s">upgrade</a> to WordPress 3.5 to use media fields', 'thim' ), admin_url( 'update-core.php' ) );
					break;
				}
				if ( !empty( $value ) ) {
					if ( is_array( $value ) ) {
				...
				$choose_title  = empty( $args['choose'] ) ? __( 'Choose Media', 'thim' ) : $args['choose'];
				$update_button = empty( $args['update'] ) ? __( 'Set Media', 'thim' ) : $args['update'];
				...
				<div class="media-field-wrapper">
					...
					<a href="#" class="media-remove-button"><?php esc_attr_e( 'Remove', 'thim' ) ?></a>
				</div>
			...
			case 'posts' :
				...
				<a href="#" class="ob-select-posts button button-secondary">
					...
					<?php esc_attr_e( 'Build Posts Query', 'thim' ) ?>
				</a>
				<?php
				break;
			...
			case 'repeater':
				...
				<div class="thim-widget-field-repeater" data-item-name="<?php echo esc_attr( $field['item_name'] ) ?>" data-repeater-name="<?php echo esc_attr( $name ) ?>" <?php echo !empty( $item_label ) ? 'data-item-label="' . esc_attr( $item_label ) . '"' : '' ?>>
					...
					<div class="thim-widget-field-repeater-add"><?php esc_attr_e( 'Add', 'thim' ) ?></div>
				</div>
				<?php
				break;
			case 'widget' :
			...
			default:
				?><?php esc_attr_e( 'Unknown Field', 'thim' ) ?><?php
				break;
		}
