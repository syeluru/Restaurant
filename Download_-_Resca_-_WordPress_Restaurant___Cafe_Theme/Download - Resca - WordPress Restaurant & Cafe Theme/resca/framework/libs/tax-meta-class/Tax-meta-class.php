...
if (!class_exists('Tax_Meta_Class')) :
    ...
    class Tax_Meta_Class {
        ...
        public function __construct($meta_box) {
            ...
            $this->SelfPath = get_template_directory_uri() . '/framework/libs/tax-meta-class';
            ...
            //     $this->SelfPath = get_template_directory_uri() . '/inc/Tax-meta-class';
            //   }else{
        ...
        public function load_scripts_styles() {
            ...
            // Get Plugin Pat
            // $this->SelfPath = get_template_directory_uri.'/inc/Tax-Meta_Class';
            $plugin_path = $this->SelfPath;
            // $plugin_path = get_template_directory_uri.'/inc/Tax-Meta_Class';
        ...
        public function insert_images() {
            ...
            foreach ($_POST['attachments'] as $attachment_id => $attachment) {
                ...
                $li .= "<a title='" . esc_attr__('Delete this image', 'thim') . "' class='at-delete-file' href='#' rel='{$nonce}|{$term_id}|{$id}|{$attachment_id}'><img src='" . esc_url($this->SelfPath) . "/images/delete-16.png' alt='" . esc_attr__('Delete', 'thim') . "' /></a>";
        ...
        public function wp_ajax_delete_image() {
            ...
            if ($ok) {
                ...
                echo json_encode(array('message' => __('Cannot delete file. Something\'s wrong.', 'thim')));
                die();
            }
        }
        ...
        public function check_field_date() {
            ...
            if ($this->has_field('date') && $this->is_edit_page()) {
                ...
                wp_enqueue_script('tmc-jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/jquery-ui.min.js', array('jquery'), true);
            }
        }
        ...
        <span class="at-re-toggle"><img src="';
                    if ($this->_Local_images) {
                        echo ent2ncr($plugin_path . '/images/edit.png');
                    } else {
        ...
        <img src="';
                    if ($this->_Local_images) {
                        echo ent2ncr($plugin_path . '/images/remove.png');
                    } else {
                        echo 'http://i.imgur.com/g8Duj.png';
                    }
                    echo '" alt="' . __('Remove', 'thim') . '" title="' . __('Remove', 'thim') . '" id="remove-' . $field['id'] . '"></div>';
                    $c = $c + 1;
                }
            ...
            echo '<img src="';
            if ($this->_Local_images) {
                echo ent2ncr($plugin_path . '/images/add.png');
            } else {
                echo 'http://i.imgur.com/w5Tuc.png';
            }
            echo '" alt="' . __('Add', 'thim') . '" title="' . esc_attr__('Add', 'thim') . '" id="add-' . $field['id'] . '"><br/></div>';
            ...
            echo '" alt="' . esc_attr__('Remove', 'thim') . '" title="' . esc_attr__('Remove', 'thim') . '" id="remove-' . $field['id'] . '"></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
        ...
        public function show_field_file($field, $meta) {
            ...
            if (!empty($meta) && count($meta) > 0 && !$this->is_array_empty($meta)) {
                ...
                echo '<div style="margin-bottom: 10px"><strong>' . __('Uploaded files', 'thim') . '</strong></div>';
                echo '<ol class="at-upload">';
                foreach ($meta as $att) {
                    // if (wp_attachment_is_image($att)) continue; // what's image uploader for?
                    echo "<li>" . wp_get_attachment_link($att, '', false, false, ' ') . " (<a class='at-delete-file' href='#' rel='{$nonce}||{$field['id']}|{$att}'>" . __('Delete', 'thim') . "</a>)</li>";
                }
                echo '</ol>';
            }
      ...
      <strong>" . __('Upload new files', 'thim') . "</strong>\n
    </div>\n";
            echo "<div class='new-files'>\n
      <div class='file-input'>\n
        <input type='file' name='{$field['id']}[]' />\n
      </div><!-- End .file-input -->\n
      <a class='at-add-file button' href='#'>" . __('Add more files', 'thim') . "</a>\n
      </div><!-- End .new-files -->\n";
            echo "</td>";
            $this->show_field_end($field, $meta);
        }
        ...
        public function show_field_color($field, $meta) {
            ...
            echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . __('Select a color', 'thim') . "'/>";
